function CTrackMenu(iLevel, aScore) {
  ...
  this._init = function (iLevel, aScore) {
    ...
    for (var i = 0; i < NUM_TRACKS_PER_WORLD; i++) {
      ...
      _aStageButton[i].addEventListenerWithParams(
        ON_MOUSE_UP,
        this._onStageBut,
        this,
        i,
  ...
  this._setStageInfo = function () {
    ...
    if (iCurIndexActive < NUM_TRACKS_PER_WORLD) {
      ...
      _aStageButton[iCurIndexActive].addScore(
        LEVEL_INFO[iLevel * NUM_TRACKS_PER_WORLD + iCurIndexActive].time,
      );
